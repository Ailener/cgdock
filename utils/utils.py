...
def construct_data_from_graph_gvp_mean(args, protein_node_xyz, protein_seq,
                                 ...
                                 input_atom_edge_attr_list, LAS_edge_index, rdkit_coords, 
                                 compound_coords_init_mode='pocket_center_rdkit', 
                                 includeDisMap=True, pdb_id=None, group='train', 
                                 seed=42, data_path=None, contactCutoff=8.0, 
                                 pocket_radius=20, interactionThresholdDistance=10, compoundMode=1, 
                                 add_noise_to_com=None, use_whole_protein=False, 
                                 use_compound_com_as_pocket=True, chosen_pocket_com=None, 
                                 random_rotation=False, pocket_idx_no_noise=True, protein_esm2_feat=None):
    ...
    dis_map = scipy.spatial.distance.cdist(input_node_xyz.cpu().numpy(), coords)   shape[n_pocket,n_compound]
    ...
    elif compound_coords_init_mode == 'pocket_center_rdkit':   # True
        if random_rotation:
    ...
    data['complex', 'c2c', 'complex'].edge_index = input_atom_edge_list[:,
                                                                        :2].long().t().contiguous() + 1  
    if compound_coords_init_mode == 'redocking' or compound_coords_init_mode == 'redocking_no_rotate':
        data['complex', 'LAS', 'complex'].edge_index = torch.nonzero(
            torch.ones(n_compound, n_compound)).t() + 1
    else:
        data['complex', 'LAS', 'complex'].edge_index = LAS_edge_index + \
            1 
    ...
    data['complex_whole_protein', 'c2c', 'complex_whole_protein'].edge_index = input_atom_edge_list[:,
                                                                                                    :2].long().t().contiguous() + 1 
    if compound_coords_init_mode == 'redocking' or compound_coords_init_mode == 'redocking_no_rotate':
        data['complex_whole_protein', 'LAS', 'complex_whole_protein'].edge_index = torch.nonzero(
            torch.ones(n_compound, n_compound)).t() + 1
    else:

        data['complex_whole_protein', 'LAS',
             'complex_whole_protein'].edge_index = LAS_edge_index + 1

    data['compound'].node_coords = coords_init  
    data['compound'].rdkit_coords = rdkit_coords
    data['compound_atom_edge_list'].x = (input_atom_edge_list[:, :2].long(
    ).contiguous() + 1).clone()  
    data['LAS_edge_list'].x = data['complex', 'LAS',
                                   'complex'].edge_index.clone().t()  

    data.node_xyz_whole = protein_node_xyz  
    data.coords_center = torch.tensor(
        com, dtype=torch.float).unsqueeze(0)  
...
def evaluate_mean_pocket_cls_coord_multi_task(accelerator, args, data_loader, model, com_coord_criterion, criterion, pocket_cls_criterion, pocket_coord_criterion, pocket_radius_criterion, relative_k, device, pred_dis=False, info=None, saveFileName=None, use_y_mask=False, skip_y_metrics_evaluation=False, stage=1):
    ...
    for data in data_iter:
        ...
        pocket_radius_pred_loss = 0.05*pocket_radius_criterion(pocket_radius_pred.squeeze(1), data.ligand_radius.to(pocket_radius_pred.dtype))
