...
class CGDockDataSet(Dataset):
    def __init__(self, root, data=None, protein_dict=None, compound_dict=None, proteinMode=0, compoundMode=1,
                ...
                transform=None, pre_transform=None, pre_filter=None, noise_for_predicted_pocket=5.0, 
                test_random_rotation=False, pocket_idx_no_noise=True, use_esm2_feat=False):
        self.data = data
    ...
    def processed_file_names(self):
        return ['data.pt', 'protein_1d_3d.lmdb', 'compound_LAS_edge_index.lmdb', 'compound_rdkit_coords.pt', 'protein_feature.lmdb']
    ...
    def get(self, idx):
        ...
        use_whole_protein = line['use_whole_protein'] if "use_whole_protein" in line.index else self.use_whole_protein  #bool
        ...
        elif group == 'test' and self.test_random_rotation:   # false
            random_rotation = True
        else:
        ...
        if self.proteinMode == 0:
            ...
            if self.use_esm2_feat:
                with self.protein_esm2_feat.begin() as txn:
                    ...
                    protein_esm2_feat = torch.from_numpy(protein_esm2_feat)  
            else:
                protein_esm2_feat = None

        name = line['compound_name']
        rdkit_coords = self.compound_rdkit_coords[name]  # 配体rdkit坐标 shape[n_compound,3] 初始化配体构像，随机产生一个低能量的配体对象
        ...
        #   配体坐标       配体点特征                配体边               配体边特征                                    边索引
        #[n_compound,3] [n_compound,56]      [n_comp_edge,2+1]      [n_comp_edge,19]       [n_compound,n_compound,16]  [2,n_LASedge]
        if self.proteinMode == 0:
            data, input_node_list, keepNode = construct_data_from_graph_gvp_mean(self.args, protein_node_xyz, protein_seq, 
                                ...
                                compound_coords_init_mode=self.compound_coords_init_mode, 
                                contactCutoff=self.contactCutoff, includeDisMap=self.predDis,
                                pocket_radius=self.pocket_radius, add_noise_to_com=add_noise_to_com, 
                                use_whole_protein=use_whole_protein, pdb_id=name, group=group, seed=self.seed, 
                                data_path=self.pre, use_compound_com_as_pocket=use_compound_com, 
                                chosen_pocket_com=pocket_com, compoundMode=self.compoundMode, 
                                random_rotation=random_rotation, pocket_idx_no_noise=self.pocket_idx_no_noise,
                                protein_esm2_feat=protein_esm2_feat)
           #data   口袋点坐标      属于口袋的点
           #     [n_pocket,3]   [n_protein]
        ...
        # logger.info(f'keepNode:{keepNode}')
...
def get_data(args, logger, addNoise=None, use_whole_protein=False, compound_coords_init_mode='pocket_center_rdkit', pre="PDBbind_data/pdbbind2020"):
    if args.data == "0":
        ...
        logger.log_message(f"protein feature based on esm2")  # 修改
        add_noise_to_com = float(addNoise) if addNoise else None

        new_dataset = CGDockDataSet(f"{pre}/dataset", add_noise_to_com=add_noise_to_com, 
                                    use_whole_protein=use_whole_protein, 
                                    compound_coords_init_mode=compound_coords_init_mode, 
                                    pocket_radius=args.pocket_radius, 
                                    noise_for_predicted_pocket=args.noise_for_predicted_pocket, 
                                    test_random_rotation=args.test_random_rotation, 
                                    pocket_idx_no_noise=args.pocket_idx_no_noise, 
                                    use_esm2_feat=args.use_esm2_feat, seed=args.seed, 
                                    pre=pre, args=args)
     
        # load compound features extracted using torchdrug.
        # c_length: number of atoms in the compound  化合物/配体中的原子数小于100，原生接触点数量大于5，使用化合物质心作为口袋中心
