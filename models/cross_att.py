...
class CrossAttentionModule(nn.Module):
    ...
    def forward(self, 
        ...
        pair_embed = pair_embed + self.inter_layer(p_embed_batched, c_embed_batched, p_mask, c_mask)[0] #更新点对嵌入

        pair_embed = self.pair_transition(pair_embed) * pair_mask.to(torch.float).unsqueeze(-1)
        return p_embed_batched, c_embed_batched, pair_embed       #[bs,glb_p||n_protein,dd][bs,glb_c||n_compound,dd][bs,glb_p||n_protein,glb_c||n_compound,dd]
...
class RowAttentionBlock(nn.Module):
    ...
    def forward(self, node_embed_i, node_embed_j, pair_embed, pair_mask, node_mask_i):
        if not self.rm_layernorm:                               # *:bs   H:num of head
        ...
        return node_embed_i  #[bs,I,C_node]
