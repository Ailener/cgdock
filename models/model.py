...
class pocket_MLP(torch.nn.Module):
...
class IaBNet_mean_and_pocket_prediction_cls_coords_dependent(torch.nn.Module):
    def __init__(self, args, embedding_channels=128, pocket_pred_embedding_channels=128):
        ...
        self.protein_to_pocket = pocket_MLP(embedding_channels=embedding_channels, n=4, out_channels=1) 
        self.pocket_radius_head =  pocket_MLP(embedding_channels=embedding_channels, n=4, out_channels=1) 
    ...
    def forward(self, data, stage=1, train=False):
        ...
        batched_protein_emb_whole_protein = self.protein_linear_whole_protein(data['protein_whole'].node_feats).float()           
       
        for i in range(complex_batch_whole_protein.max()+1):      #i=0,2,……,bs-1
            if i == 0:
        ...
        num_atoms = compound_emb_mask.sum(dim=1, dtype=torch.float32) 

        pocket_radius_pred = self.pocket_radius_head(compound_emb_batch.sum(dim=1)).relu()+torch.sqrt(num_atoms).unsqueeze(1)   
        ...
        return compound_coords_out, compound_batch, y_pred, y_pred_by_coords, pocket_cls_pred, pocket_cls, protein_out_mask_whole, protein_coords_batched_whole, pred_pocket_center, dis_map, keepNode_less_5, pocket_radius_pred
...
def get_model(args, logger):
    if args.mode == 5:
    return IaBNet_mean_and_pocket_prediction_cls_coords_dependent(args, args.hidden_size, args.pocket_pred_hidden_size)
